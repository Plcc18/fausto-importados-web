...
export function NotificationsPanel({ open, onOpenChange }: NotificationsPanelProps) {
  ...
  const handleClearHistory = async () => {
    ...
    try {
      ...
      // Remove apenas COMPLETED e CANCELLED do estado local.
      // Pedidos PENDING não são tocados — o backend também não os apaga.
    ...
    <>
          ...
          <SheetHeader className="shrink-0 border-b px-6 py-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                <button onClick={fetchOrders} title="Atualizar" className="flex h-9 w-9 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">
                  <RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} />
                </button>
                <button onClick={() => setClearConfirmOpen(true)} title="Limpar histórico" className="flex h-9 w-9 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-muted hover:text-destructive">
                  <Trash2 className="h-4 w-4" />
                </button>
                <button onClick={() => onOpenChange(false)} title="Fechar" className="flex h-9 w-9 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">
                  <X className="h-5 w-5" />
                </button>
              </div>
            </div>
            ...
            <div className="flex gap-2 mt-3 flex-wrap">
              {(["ALL", "PENDING", "COMPLETED", "CANCELLED"] as const).map((s) => (
                <Button key={s} variant={filter === s ? "default" : "outline"} size="sm" onClick={() => setFilter(s)} className="text-xs h-7">
                      ...
                      <div className="space-y-1">
                        {order.items.map((item) => (
                          <div key={item.id} className="flex justify-between text-sm">
                            <span className="text-muted-foreground">{item.quantity}x {item.productName} {item.productSize}ml</span>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {order.status === "PENDING" && (
                          <div className="flex gap-2">
                            <Button size="sm" variant="outline" className="gap-1 text-red-600 border-red-200 hover:bg-red-50 dark:border-red-800 dark:hover:bg-red-950/30" onClick={() => handleCancel(order.id)}>
                              <X className="h-3.5 w-3.5" />
                              Cancelar
                            </Button>
                            <Button size="sm" className="gap-1 bg-green-600 hover:bg-green-700" onClick={() => handleComplete(order.id)}>
      ...
      <Dialog open={clearConfirmOpen} onOpenChange={setClearConfirmOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader><DialogTitle>Limpar histórico</DialogTitle></DialogHeader>
          <p className="text-sm text-muted-foreground">
            Isso vai apagar todos os pedidos <strong>concluídos e cancelados</strong>. Pedidos pendentes não serão removidos.
          </p>
          <DialogFooter>
